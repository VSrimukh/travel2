...
def create_optimized_prompt(template, user_prompt,preferences=None):
    ...
    if preferences:
        if preferences.get('duration'):
            duration = str(preferences['duration'])
        if preferences.get('destination'):
            destination = preferences['destination']
        if preferences.get('travelers'):
            travelers = str(preferences['travelers']) + " travelers"
...
def generate_budget():
    ...
    try:
        ...
        full_prompt = create_optimized_prompt(BUDGET_TEMPLATE, user_prompt,preferences)
...
def generate_fallback_itinerary(user_prompt,preferences=None):
    ...
    if preferences:
        duration = preferences.get('duration', duration)
...
def generate_fallback_budget(user_prompt,preferences=None):
    ...
    if preferences:
        duration = preferences.get('duration', duration)
        travelers = preferences.get('travelers', travelers)
